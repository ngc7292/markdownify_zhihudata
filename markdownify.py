...
class MarkdownConverter(object):
    ...
    def convert_soup(self, soup):
        return self.process_tag(soup, convert_as_inline=self.options['convert_as_inline'], children_only=True)
    ...
    def process_tag(self, node, convert_as_inline, children_only=False):
        ...
        for el in node.children:
            if isinstance(el, Comment) or isinstance(el, Doctype):
                ...
                t = self.process_text(el)
                text += t
                text_list.append(t)
            else:
                t, t_list = self.process_tag(el, convert_children_as_inline)
                text += t
                text_list.extend(t_list)

        if not children_only:
            convert_fn = getattr(self, 'convert_%s' % node.name, None)
            if convert_fn and self.should_convert_tag(node.name):
                t = convert_fn(node, text, convert_as_inline)
                text = t
                text_list = [t]
                # print(convert_fn, t)
    ...
    def convert_a(self, el, text, convert_as_inline):
        ...
        # convert related url to absolute
        if href is not None:
            res = re.search(r"^(http://|https://).", href)
            if res == None and href[0] == '/':
    ...
    def convert_img(self, el, text, convert_as_inline):
        ...
        if re.search(r"^[http://|https://].", src) is None: return ''
        if re.search(r"[https://www.zhihu.com/equation].", src) is not None:
            # zhihu will convert equation and upload to the url https://www.zhihu.com/equation
            # in this repo, we extract the equation and convert to markdown style like $ X \eq Y $  
            alt = alt.strip()
            if alt is None or alt == "": return ""
            return '$ %s $' % alt
        # if re.search(r"", src): 
...
def reclean_raw_data(raw_data):
    need_remove_sambol_list = re.findall(r"\\x[A-Z0-9a-z][A-Z0-9a-z]", raw_data)

    for remove_sambol in need_remove_sambol_list:
        raw_data = raw_data.replace(remove_sambol, "")
